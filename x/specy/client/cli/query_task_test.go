...
func networkWithTaskObjects(t *testing.T, n int) (*network.Network, []types.Task) {
	t.Helper()
	...
	return network.New(t, cfg), state.TaskList
}

func TestShowTask(t *testing.T) {
	net, objs := networkWithTaskObjects(t, 2)
	...
	for _, tc := range []struct {
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if tc.err != nil {
				...
				require.NoError(t, err)
				var resp types.QueryGetTaskResponse
...
func TestListTask(t *testing.T) {
	net, objs := networkWithTaskObjects(t, 5)
	...
	t.Run("ByOffset", func(t *testing.T) {
		...
		for i := 0; i < len(objs); i += step {
			...
			out, err := clitestutil.ExecTestCLICmd(ctx, cli.CmdListTask(), args)
			require.NoError(t, err)
			var resp types.QueryAllTaskResponse
			require.NoError(t, net.Config.Codec.UnmarshalJSON(out.Bytes(), &resp))
